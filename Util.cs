namespace ConsoleApp1
{
    public class Util
    {
        public string BuscarExtenso(int valor, string separador)
        {
            string numeroPorExtenso = string.Empty;
            if (valor < 20)
            {
                numeroPorExtenso = RetornarValorString(valor);
            }
            ...
            if (valor > 19)
            {
                ...
                if (len == 2)
                {
                    ...
                    numeroPorExtenso = RetornarValorString(ValorPrimario) + (ValorSecundario > 0 ? separador + RetornarValorString(ValorSecundario) : "");
                }
                else if (len == 3)
                {
                    int ValorPrimario = int.Parse(valor.ToString().Substring(0, 1));
                    ...
                    numeroPorExtenso = RetornarValorString(ValorPrimario)
                                      + (ValorSecundario > 0 ? separador + RetornarValorString(ValorSecundario) : "")
                                      + (ValorTerciario > 0 ? separador + RetornarValorString(ValorTerciario) : "");
                }
            }
        ...
        public string RetornarValorString(int identificador)
        {
